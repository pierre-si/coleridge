...
for i in range(n_pub):
    for j in range(i + 1, n_pub):
        word_sim[i][j] = id_to_doc[df_title_sets.index[i]].similarity(
            id_to_doc[df_title_sets.index[j]]
        )
# %%
...
for jaccard_val in np.unique(jaccard_matrix)[1:]:
    print(
        "Average similarity among the",
        np.sum(jaccard_matrix == jaccard_val),
        "docs with Jaccard index",
        "{:.2f}".format(jaccard_val),
        ":",
        "{:.3f}".format(mean(word_sim[jaccard_matrix == jaccard_val])),
    )
# %%
